...
BlazeFaceDetector::BlazeFaceDetector() : initialized_(false), use_dnn_model_(false) {
}
...
bool BlazeFaceDetector::initialize(const std::string& model_path) {
    try {
        std::cout << "Initializing BlazeFace detector (OpenCV version)..." << std::endl;
        
        // Try to load DNN model first
        if (!model_path.empty() && initializeDNNModel(model_path)) {
            use_dnn_model_ = true;
            std::cout << "Using DNN model for face detection" << std::endl;
        } else {
            // Fallback to cascade classifier
            if (!initializeCascadeClassifier()) {
                std::cerr << "Failed to initialize cascade classifier" << std::endl;
                return false;
            }
            use_dnn_model_ = false;
            std::cout << "Using cascade classifier for face detection" << std::endl;
        }
...
bool BlazeFaceDetector::initializeDNNModel(const std::string& model_path) {
    try {
        // Try to load a Caffe or TensorFlow model that OpenCV DNN can handle
        // For now, we'll use OpenCV's built-in face detection model
        
        std::string prototxt_path = "./models/deploy.prototxt";
        std::string caffemodel_path = "./models/res10_300x300_ssd_iter_140000.caffemodel";
        
        // Download model files if they don't exist
        if (!std::filesystem::exists(prototxt_path) || !std::filesystem::exists(caffemodel_path)) {
            std::cout << "Downloading face detection model..." << std::endl;
            ...
            if (curl) {
                std::ofstream prototxt_file(prototxt_path, std::ios::binary);
                curl_easy_setopt(curl, CURLOPT_URL, 
                    "https://raw.githubusercontent.com/opencv/opencv/master/samples/dnn/face_detector/deploy.prototxt");
                curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, WriteCallback);
                curl_easy_setopt(curl, CURLOPT_WRITEDATA, &prototxt_file);
                ...
                // Download caffemodel
                std::ofstream model_file(caffemodel_path, std::ios::binary);
                curl_easy_setopt(curl, CURLOPT_URL,
                    "https://github.com/opencv/opencv_3rdparty/raw/dnn_samples_face_detector_20170830/res10_300x300_ssd_iter_140000.caffemodel");
                curl_easy_setopt(curl, CURLOPT_WRITEDATA, &model_file);
...
bool BlazeFaceDetector::initializeCascadeClassifier() {
    try {
        ...
        if (!std::filesystem::exists(cascade_path)) {
            std::cout << "Downloading Haar cascade..." << std::endl;
            std::filesystem::create_directories("./models");
            
            CURL* curl = curl_easy_init();
            if (curl) {
                std::ofstream file(cascade_path, std::ios::binary);
                curl_easy_setopt(curl, CURLOPT_URL,
                ...
                if (res == CURLE_OK && face_cascade_.load(cascade_path)) {
                    return true;
                }
            }
        }
        ...
        std::cerr << "Error loading cascade classifier: " << e.what() << std::endl;
        return false;
    }
}
...
std::vector<FaceDetection> BlazeFaceDetector::detectFaces(const cv::Mat& image) {
    ...
    if (use_dnn_model_) {
        ...
        return detectFacesCascade(image);
    }
}

std::vector<FaceDetection> BlazeFaceDetector::detectFacesDNN(const cv::Mat& image) {
    std::vector<FaceDetection> detections;
    
    try {
        // Prepare input blob
        cv::Mat blob = cv::dnn::blobFromImage(image, 1.0, cv::Size(INPUT_WIDTH, INPUT_HEIGHT),
                                              cv::Scalar(104, 177, 123), false, false);
        
        net_.setInput(blob);
        cv::Mat detection_mat = net_.forward();
        
        // Process detections
        cv::Mat detection_mat_reshaped = detection_mat.reshape(1, detection_mat.total() / 7);
        
        for (int i = 0; i < detection_mat_reshaped.rows; ++i) {
            float confidence = detection_mat_reshaped.at<float>(i, 2);
            ...
            float x1 = detection_mat_reshaped.at<float>(i, 3);
            float y1 = detection_mat_reshaped.at<float>(i, 4);
            float x2 = detection_mat_reshaped.at<float>(i, 5);
            float y2 = detection_mat_reshaped.at<float>(i, 6);
...
std::vector<FaceDetection> BlazeFaceDetector::detectFacesCascade(const cv::Mat& image) {
    std::vector<FaceDetection> detections;
    ...
    try {
        ...
        for (const auto& face : faces) {
            FaceDetection det;
            det.bbox.x = static_cast<float>(face.x) / image.cols;
            det.bbox.y = static_cast<float>(face.y) / image.rows;
            det.bbox.width = static_cast<float>(face.width) / image.cols;
            det.bbox.height = static_cast<float>(face.height) / image.rows;
            det.confidence = 0.9f; // Cascade doesn't provide confidence
        ...
        std::cerr << "Error in cascade face detection: " << e.what() << std::endl;
        return {};
    }
}
...
std::vector<cv::Point3f> BlazeFaceDetector::get3DModelPoints() {
    std::vector<cv::Point3f> model_points;
    
    model_points.push_back(cv::Point3f(0.0f, 0.0f, 0.0f));          // Nose tip
    model_points.push_back(cv::Point3f(0.0f, -330.0f, -65.0f));     // Chin
    model_points.push_back(cv::Point3f(-225.0f, 170.0f, -135.0f));  // Left eye corner
    model_points.push_back(cv::Point3f(225.0f, 170.0f, -135.0f));   // Right eye corner
    model_points.push_back(cv::Point3f(-150.0f, -150.0f, -125.0f)); // Left mouth corner
    model_points.push_back(cv::Point3f(150.0f, -150.0f, -125.0f));  // Right mouth corner
...
HeadPose BlazeFaceDetector::calculateHeadPose(const FaceDetection& face, const cv::Size& image_size) {
    ...
    // Roll: estimate based on face aspect ratio
...
HeadPose BlazeFaceDetector::calculateHeadPoseFromLandmarks(const cv::Mat& image, const cv::Rect& face_rect) {
    HeadPose pose;
    
    // Extract landmarks
    auto landmarks = extractLandmarks(image, face_rect);
    
    if (landmarks.size() < 6) {
        // Not enough landmarks, use simple method
        FaceDetection det;
        det.bbox = cv::Rect2f(
            static_cast<float>(face_rect.x) / image.cols,
            static_cast<float>(face_rect.y) / image.rows,
            static_cast<float>(face_rect.width) / image.cols,
            static_cast<float>(face_rect.height) / image.rows
        );
        return calculateHeadPose(det, image.size());
    }
    
    // Use PnP to estimate head pose
    std::vector<cv::Point3f> model_points = get3DModelPoints();
    std::vector<cv::Point2f> image_points = get2DImagePoints(landmarks);
    ...
    if (!singular) {
        pose.pitch = std::atan2(rotation_matrix.at<double>(2, 1), rotation_matrix.at<double>(2, 2)) * 180.0 / CV_PI;
        pose.yaw = std::atan2(-rotation_matrix.at<double>(2, 0), sy) * 180.0 / CV_PI;
        pose.roll = std::atan2(rotation_matrix.at<double>(1, 0), rotation_matrix.at<double>(0, 0)) * 180.0 / CV_PI;
    } else {
        pose.pitch = std::atan2(-rotation_matrix.at<double>(1, 2), rotation_matrix.at<double>(1, 1)) * 180.0 / CV_PI;
        pose.yaw = std::atan2(-rotation_matrix.at<double>(2, 0), sy) * 180.0 / CV_PI;
        pose.roll = 0;
    }
...
std::vector<cv::Point2f> BlazeFaceDetector::extractLandmarks(const cv::Mat& image, const cv::Rect& face_rect) {
    std::vector<cv::Point2f> landmarks;
    
    // Simple landmark estimation based on face rectangle
    // These are approximate positions for 6 key points
    float x = face_rect.x;
    float y = face_rect.y;
    float w = face_rect.width;
    float h = face_rect.height;
    
    // Nose tip
    landmarks.push_back(cv::Point2f(x + w * 0.5f, y + h * 0.6f));
    // Chin
    landmarks.push_back(cv::Point2f(x + w * 0.5f, y + h * 0.95f));
    // Left eye corner
    landmarks.push_back(cv::Point2f(x + w * 0.25f, y + h * 0.35f));
    // Right eye corner
    landmarks.push_back(cv::Point2f(x + w * 0.75f, y + h * 0.35f));
    // Left mouth corner
    landmarks.push_back(cv::Point2f(x + w * 0.35f, y + h * 0.75f));
    // Right mouth corner
    landmarks.push_back(cv::Point2f(x + w * 0.65f, y + h * 0.75f));
    
    return landmarks;
}

std::vector<cv::Point2f> BlazeFaceDetector::get2DImagePoints(const std::vector<cv::Point2f>& landmarks) {
    // Return first 6 landmarks if available
    if (landmarks.size() >= 6) {
        return std::vector<cv::Point2f>(landmarks.begin(), landmarks.begin() + 6);
    }
    return landmarks;
}
...
float BlazeFaceDetector::performLivenessCheck(const std::vector<cv::Mat>& frames, 
    ...
    // Check 3: Face position variation
    if (detections.size() > 1) {
...
VideoAnalysis BlazeFaceDetector::analyzeVideo(const std::string& video_path_or_url) {
    ...
    for (const auto& frame : frames) {
        ...
        if (face_opt.has_value()) {
            ...
            // Calculate pose
            cv::Rect pixel_rect = face.toPixelCoords(frame.cols, frame.rows);
            HeadPose pose = calculateHeadPoseFromLandmarks(frame, pixel_rect);
            analysis.poses.push_back(pose);
            
            // Track face size
            float face_size = static_cast<float>(pixel_rect.area()) / (frame.cols * frame.rows);
